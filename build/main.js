...
const drawAllObjects = (objects, canvas) => {
...
const drawStartText = (size, canvas) => {
    ...
    canvas.ctx.fillText(text, (canvas.element.width / 2) - (textSize.width / 2), (canvas.element.height / 2) + (textSize.actualBoundingBoxAscent / 2));
    canvas.ctx.fillStyle = "black";
};
const drawPlacingBall = (lineStart, lineEnd, radius, canvas) => {
    //draw ball outline
...
document.getElementById("ball-life").addEventListener("change", (e) => {
    const immortalBallsInput = e.target;
